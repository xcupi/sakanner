// Phase 3.28 Open Redirect Active Detection: real orchestrator/engine
// + real lab integration tests, against harness_vuln.go's existing
// /redirect/open/* fixtures and harness_openredirect_active.go's own
// new form/JSON/path/negative-control/chain additions. Mirrors
// lab/phase3_27_traversal_active_test.go's own registry/testVulnLab/
// runReconAgainstVulnLab pattern exactly.
package lab
...
func openRedirectActiveRegistry(t *testing.T) *detection.Registry {
	t.Helper()
	r := detection.NewRegistry()
	if err := r.Register(openredirectactive.New(OpenRedirectDestination)); err != nil {
		t.Fatalf("Register: %v", err)
	}
	return r
}

func findOpenRedirectFor(findings []models.Finding, endpoint, parameter string) (models.Finding, bool) {
	for _, f := range findings {
		if f.VulnerabilityType == "open_redirect" && f.AffectedEndpoint == endpoint && f.AffectedParameter == parameter {
			return f, true
		}
	}
...
func TestPhase3_28_QueryLocation_Finding(t *testing.T) {
	l := testVulnLab(t)
	store, job, validator := runReconAgainstVulnLab(t, l)

	x := detection.NewExecutor(validator, l.Resolver, detection.ExecutorConfig{Concurrency: 8, MaxRedirects: 5})
	e := &detection.Engine{Registry: openRedirectActiveRegistry(t), Store: store, Executor: x, Concurrency: 8, Logger: detectionLogger()}
	...
	f, found := findOpenRedirectFor(findings, "/redirect/open/vulnerable", "next")
	if !found {
		t.Fatalf("expected an open_redirect finding for /redirect/open/vulnerable's next parameter, got: %+v", findings)
	}
	if len(f.Evidence) != 2 {
...
func TestPhase3_28_FormLocation_Finding(t *testing.T) {
	l := testVulnLab(t)
	...
	if _, found := findOpenRedirectFor(findings, "/redirect/open/vulnerable-form", "next"); !found {
		t.Fatalf("expected an open_redirect finding for /redirect/open/vulnerable-form's next parameter, got: %+v", findings)
	}
}

func TestPhase3_28_PathLocation_Finding(t *testing.T) {
	l := testVulnLab(t)
	store, job, validator := runReconAgainstVulnLab(t, l)
	...
	if !foundPathParam {
		t.Fatalf("expected at least one path-location parameter to have been discovered via the real crawl (/redirect/open/next/<value>, two example links): %+v", params)
	}
	...
	found := false
	for _, f := range findings {
		if f.VulnerabilityType == "open_redirect" && strings.HasPrefix(f.AffectedEndpoint, "/redirect/open/next/") {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected an open_redirect finding for the path-location /redirect/open/next/<value> parameter, got: %+v", findings)
	}
}
...
func TestPhase3_28_JSONLocation_DirectlyPersisted_Finding(t *testing.T) {
	l := testVulnLab(t)
	...
	scanJobID := "openredirect-json-job"
	...
	if err := store.Endpoints().Create(context.Background(), models.Endpoint{
		ID: "ep-oredirect-json", ScanJobID: scanJobID, HTTPServiceID: "http1", Path: "/redirect/open/vulnerable-json", Method: "POST", Source: "crawl", CreatedAt: time.Now().UTC(),
	}); err != nil {
		t.Fatalf("create endpoint: %v", err)
	}
	if err := store.Parameters().Create(context.Background(), models.Parameter{
		ID: "p-oredirect-json", ScanJobID: scanJobID, EndpointID: "ep-oredirect-json", Name: "next", Location: "json",
		Method: "POST", Provenance: "REQUEST_INPUT", CreatedAt: time.Now().UTC(),
	}); err != nil {
		t.Fatalf("create parameter: %v", err)
	}

	x := detection.NewExecutor(validator, l.Resolver, detection.ExecutorConfig{Concurrency: 2, Timeout: 3 * time.Second, MaxRedirects: 5})
	e := &detection.Engine{Registry: openRedirectActiveRegistry(t), Store: store, Executor: x, Logger: detectionLogger(), Concurrency: 2}
	...
	if _, found := findOpenRedirectFor(findings, "/redirect/open/vulnerable-json", "next"); !found {
		t.Fatalf("expected an open_redirect finding from the JSON-body next parameter, got: %+v", findings)
	}
}

// ---------------------------------------------------------------------
// NEGATIVE: safe endpoints, negative controls, in-scope chain
// ---------------------------------------------------------------------

func TestPhase3_28_NegativeControls_NoFinding(t *testing.T) {
	l := testVulnLab(t)
	store, job, validator := runReconAgainstVulnLab(t, l)
	...
	for _, path := range []string{
		"/redirect/open/safe", "/redirect/safe-origin", "/redirect/relative-only",
		"/redirect/reflect-only", "/redirect/tracking-decoy", "/redirect/chain/in-scope",
	} {
		for _, f := range findings {
			if f.VulnerabilityType == "open_redirect" && f.AffectedEndpoint == path {
...
func TestPhase3_28_RedirectChain_OutOfScope_Finding(t *testing.T) {
	l := testVulnLab(t)
	store, job, validator := runReconAgainstVulnLab(t, l)
	...
	if _, found := findOpenRedirectFor(findings, "/redirect/chain/out-of-scope", "next"); !found {
		t.Fatalf("expected an open_redirect finding for the 2-hop /redirect/chain/out-of-scope chain, got: %+v", findings)
	}
}
...
func deepAuthOpenRedirectOrchestrator(t *testing.T, l *Lab, rules []models.ScopeRule) (*orchestrator.Orchestrator, storage.Store) {
	t.Helper()
	store, err := sqlite.New(context.Background(), ":memory:")
	if err != nil {
		t.Fatalf("sqlite.New: %v", err)
	...
	if err := registry.Register(openredirectactive.New(OpenRedirectDestination)); err != nil {
		t.Fatalf("Register(open-redirect-active): %v", err)
	}
	orch := &orchestrator.Orchestrator{
		Store:                   store,
		Pipeline:                pipeline,
		DetectionRegistry:       registry,
		DetectionExecutorConfig: detection.ExecutorConfig{Concurrency: 2, Timeout: 3 * time.Second, MaxRedirects: 5},
...
func TestPhase3_28_AuthenticatedOpenRedirect_TwoIdentities_SessionIsolated(t *testing.T) {
	l := testAuthLab(t)
	...
	runFor := func(sess *auth.Session) (models.Finding, bool) {
		orch, store := deepAuthOpenRedirectOrchestrator(t, l, rules)
		...
		return findOpenRedirectFor(findings, "/redirect-me", "next")
	}

	fA, foundA := runFor(sessA)
	fB, foundB := runFor(sessB)
	if !foundA || !foundB {
		t.Fatalf("expected both identities to independently produce an open_redirect finding for /redirect-me: foundA=%v foundB=%v", foundA, foundB)
	}
...
func TestPhase3_28_Determinism_RepeatedScans_SameFindingCount(t *testing.T) {
	l := testVulnLab(t)
	...
	for i := 1; i < len(counts); i++ {
		if counts[i] != counts[0] {
			t.Errorf("open_redirect finding count not deterministic: run 0=%d run %d=%d", counts[0], i, counts[i])
		}
	}
}
